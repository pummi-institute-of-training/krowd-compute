import imp
from config import connection, MONGODB_ENTITY_COLLECTION,MONGODB_CATEGORY_DB,MONGODB_RECOMMENDATION_DB,MONGODB_RECOMMENDATION_RESTAURANT, WEBSITE, LATEST_REVIEW_DATE
import pandas as pd
import ast
...
def fetch_city_dataset(city,country):
    ...
    try:
        total_records = entitycollection.count_documents({"city":city,"country":country,"reviews": {"$exists": True}})
        logger.info("Total Records for the city with reviews - {} is {}".format(city,total_records))
        batch_size = 250
        current_index = 0 
        all_dfs = []
        ...
        while current_index < total_records:
            ...
            results = entitycollection.find({"city": city,"country":country,"reviews": {"$exists": True}}, {"entity_url": 1, "name": 1,"cuisines":1, "city": 1,
                                                        "address": 1, "logo_url": 1,'reviews': 1, "_id": 0}).skip(current_index).limit(batch_size)
            results = list(results)
            logger.info("Fetching of city data batch at {} completed, now started creating dataframe.".format(current_index))
            sliced_df = pd.DataFrame(results)
            all_dfs.append(sliced_df)
            current_index = current_index + batch_size
            
        df = pd.concat(all_dfs)
        logger.info("City Dataframe got created successfully and it is of shape : {} \n with head \n {}".format(
            df.shape, df.head()))
        return df
...
def replace_empty_string_with_zero(val):
    if val=="":
        return 0
    return int(val)
...
def calculate_reviewer_rank(unique_reviewer_df):
    ...
    if WEBSITE=='YELP':
        ...
        reviewer_rank_key= 'reviewer_contributions'
    # unique_reviewer_df[reviewer_rank_key]=unique_reviewer_df[reviewer_rank_key].apply(replace_empty_string_with_zero)
    std = unique_reviewer_df[reviewer_rank_key].std()
    avg = unique_reviewer_df[reviewer_rank_key].mean()
    logger.info("Reviewer rank key is {} \n Average : {} \t Std : {}".format(reviewer_rank_key,avg,std))
    unique_reviewer_df['reviewer_rank']=unique_reviewer_df[reviewer_rank_key].apply(calculate_reviewer_helpfullness, args=(avg, std))
...
def calculate_review_rank(df):
    ...
    if WEBSITE == "YELP":
        ...
        review_rank_key = 'helpful_votes_of_the_review'

    df[review_rank_key]=df[review_rank_key].apply(replace_empty_string_with_zero)
    avg = df[review_rank_key].mean()
    std = df[review_rank_key].std()
    logger.info(" Review rank key is {} \n Average : {} \t Std : {} ".format(review_rank_key,avg,std))
    df['review_rank']=df[review_rank_key].apply(calculate_review_helpfullness, args=(avg, std))
    return df 
...
def create_reviewer_review_dateset(df):
    ...
    try:
        ...
        for index,row in df.iterrows():
            ...
            orig_reviews = row['reviews']
            
            orig_reviews = bytes(str(orig_reviews), encoding='utf-8')
            orig_reviews = orig_reviews.decode('utf-8')
            orig_reviews = ast.literal_eval(orig_reviews)
            
            # remove duplicate reviews from each entity reviews
            temp_id = []
            reviews = []
            for item in orig_reviews:
                if item['review_id'] in temp_id:
                    pass 
                else:
                    temp_id.append(item['review_id'])
                    reviews.append(item)
            ...
            if WEBSITE == "YELP":
                temp = temp[['reviewer_id', 'reviewer_photo_count', 'reviewer_review_count',
                'reviewer_friend_count', 'review_id', 'review_rating', 'review_date', 'crawled_date',
                'review_funny_count', 'review_useful_count', 'review_cool_count', 'review_photo_count']]
            else:
                temp = temp[['user_id', 'review_id', 'review_rating', 'review_date', 'crawled_date',
                'reviewer_contributions', 'reviewer_helpful_votes', 'helpful_votes_of_the_review']]
                temp.rename(columns={'user_id' : 'reviewer_id'}, inplace=True)
...
def multiply_krowd_score_with_wilson_score(row, wilson_score_dict):
    return row['krowd_score']*wilson_score_dict[row['entity_url']]
...
def calculate_krowd_score(df):
    ...
    df['durability'] = df['review_date'].apply(calculate_durability_factor)
    
    for col in ['review_rank', 'reviewer_rank', 'durability', 'review_rating', 'positive_reviews_count', 'rating_count']:
        df[col]=df[col].astype(float)
    df['product_of_dhs'] = df['review_rank']*df['reviewer_rank']*df['durability']
    df['product_of_dhrs'] = df['review_rank']*df['reviewer_rank']*df['durability']*df['review_rating']
    ...
    krowd_score_df['krowd_rating'] = krowd_score_df['sum_of_product_dhrs'] / krowd_score_df['sum_of_product_dhs']
    
    for index,row in krowd_score_df.iterrows():
        krowd_score_df.loc[index,'krowd_score'] = row['krowd_rating'] * wilson_score_dict[row['entity_url']]
...
def calculate_durability_factor(review_date):
    ...
    ref_date = datetime.strptime(LATEST_REVIEW_DATE,'%m/%d/%Y')
...
def save_entity_summary(summary_documents):
    ...
    records = [value for _, value in summary_documents.items()]
    
    # For fresh insertion
    rest_rec_collection.insert_many(records)

    # For update
    # for key,value in summary_documents.items():
    #     rest_rec_collection.update_one({"entity_url":key}, dict(value), upsert=True)
...
def compute_driver(city,country):
    """[This is the driver function which facilitate all the compute ]

    Args:
        city ([string]): [input city name]
    """
    OUTPUT_FOLDER = "/Users/sanjeetkumar/Documents/Krowd/Deliveries/London-July2022"
    filepath = os.path.join(OUTPUT_FOLDER,city+"-"+country+".csv")
    df = fetch_city_dataset(city,country)           
    df.to_csv(filepath)                                               
    ...
    reviewer_review_combined[reviewer_rank_key] = reviewer_review_combined[reviewer_rank_key].apply(replace_empty_string_with_zero)
    unique_reviewer_df = reviewer_review_combined.sort_values(reviewer_rank_key,ascending=False)
    unique_reviewer_df = unique_reviewer_df.drop_duplicates('reviewer_id', keep='first')
    print(unique_reviewer_df.columns.tolist())
    ...
    final_df = pd.DataFrame([value for key,value in summary_documents.items()])
    final_df.to_csv(os.path.join(OUTPUT_FOLDER,'summary-london.csv'),index=False,header=True)
