...
def load_bigram_finder():
    ...
    print("replaced dict",replace_dict)
    return replace_dict
...
if __name__ == '__main__':
    ...
    dict_to_replace = load_bigram_finder()
    reg_dict = re.compile("|".join(map(re.escape, dict_to_replace.keys())))
    paths_sorted = map(lambda x: x.replace(os.path.join(ROOT_DIR,ROOT_FOLDER+REVIEW),os.path.join(ROOT_DIR,ROOT_FOLDER+FORMATTED)), paths_sorted)  #paths_sorted is the path to the cleaned reviews folder i.e 'trip_london_formatted'
    p.map(writeBigramFile, paths_sorted)
    print("Bigram creation completed and it took : "+str((time.time()-start)/60.0))
